...
void Solid::_FaceNormal(MeshLib::Solid *pMesh) {
	for( MeshLib::SolidFaceIterator fiter( pMesh ); !fiter.end(); ++ fiter ) {
		...
		if( f->trait() == NULL ) {
			MeshLib::CFaceTrait * ft = new MeshLib::CFaceTrait;
			if( ft == NULL ) return;
			MeshLib::add_trait<MeshLib::CFaceTrait,MeshLib::Face>(f,ft);
		}
...
void Solid::_VertexNormal(MeshLib::Solid *pMesh) {
	...
	{
		MeshLib::Face * f = *fiter;
		MeshLib::CFaceTrait * pfnt = pTrait<MeshLib::CFaceTrait,MeshLib::Face>( f );
		del_trait<MeshLib::CFaceTrait,MeshLib::Face>(f, pfnt);
	}
#else
